...
try {
    ...
    document.getElementById("gameScreen").addEventListener("touchstart", (event) => { //touch started = mouse down
        vm.setMouseDown(true);
        //also move the mouse after setting mouse down.
        var pos = getMousePos(document.getElementById("gameScreen"), {
    ...
    document.getElementById("gameScreen").addEventListener("touchend", (event) => { //touch ended = mouse up
        vm.setMouseDown(false);
        event.preventDefault();
    });
    //mouse movement for touch screen
    document.getElementById("gameScreen").addEventListener("touchmove", (event) => { //drag/touch
        //also move the mouse after setting mouse down.
        var pos = getMousePos(document.getElementById("gameScreen"), {
            clientX: event.touches[event.touches.length - 1].clientX,
            clientY: event.touches[event.touches.length - 1].clientY
